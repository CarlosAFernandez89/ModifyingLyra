...
void ULyraCheatManager::ManaDamageSelf(float DamageAmount)
{
	if (ULyraAbilitySystemComponent* LyraASC = GetPlayerAbilitySystemComponent())
	{
		ApplySetByCallerManaDamage(LyraASC, DamageAmount);
	}
}
...
void ULyraCheatManager::ManaDamageTarget(float DamageAmount)
{
	if (ALyraPlayerController* LyraPC = Cast<ALyraPlayerController>(GetOuterAPlayerController()))
	{
		if (LyraPC->GetNetMode() == NM_Client)
		{
			// Automatically send cheat to server for convenience.
			LyraPC->ServerCheat(FString::Printf(TEXT("DamageManaTarget %.2f"), DamageAmount));
			return;
		}
		...
		if (ULyraAbilitySystemComponent* LyraTargetASC = Cast<ULyraAbilitySystemComponent>(UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(TargetActor)))
		{
			ApplySetByCallerManaDamage(LyraTargetASC, DamageAmount);
		}
	}
}
...
void ULyraCheatManager::ApplySetByCallerManaDamage(ULyraAbilitySystemComponent* LyraASC, float DamageAmount)
{
	check(LyraASC);

	TSubclassOf<UGameplayEffect> ManaDamageGE = ULyraAssetManager::GetSubclass(ULyraGameData::Get().ManaDamageGameplayEffect_SetByCaller);
	FGameplayEffectSpecHandle SpecHandle = LyraASC->MakeOutgoingSpec(ManaDamageGE, 1.0f, LyraASC->MakeEffectContext());
...
void ULyraCheatManager::ManaHealSelf(float HealAmount)
{
	if (ULyraAbilitySystemComponent* LyraASC = GetPlayerAbilitySystemComponent())
	{
		ApplySetByCallerManaHeal(LyraASC, HealAmount);
	}
}
...
void ULyraCheatManager::ManaHealTarget(float HealAmount)
{
	if (ALyraPlayerController* LyraPC = Cast<ALyraPlayerController>(GetOuterAPlayerController()))
	{
		FHitResult TargetHitResult;
		...
		if (ULyraAbilitySystemComponent* LyraTargetASC = Cast<ULyraAbilitySystemComponent>(UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(TargetActor)))
		{
			ApplySetByCallerManaHeal(LyraTargetASC, HealAmount);
		}
	}
}
...
void ULyraCheatManager::ApplySetByCallerManaHeal(ULyraAbilitySystemComponent* LyraASC, float HealAmount)
{
	check(LyraASC);

	TSubclassOf<UGameplayEffect> ManaHealGE = ULyraAssetManager::GetSubclass(ULyraGameData::Get().ManaHealGameplayEffect_SetByCaller);
	FGameplayEffectSpecHandle SpecHandle = LyraASC->MakeOutgoingSpec(ManaHealGE, 1.0f, LyraASC->MakeEffectContext());
 		...
 		if (const ULyraPawnExtensionComponent* PawnExtComp = ULyraPawnExtensionComponent::FindPawnExtensionComponent(LyraPC->GetPawn()))
		{
			if (PawnExtComp->HasReachedInitState(LyraGameplayTags::InitState_GameplayReady))
			{
				if (ULyraHealthComponent* HealthComponent = ULyraHealthComponent::FindHealthComponent(LyraPC->GetPawn()))
				{
					...
					// Doing this to the other attributes just for visuals.
					if(ULyraManaComponent* ManaComponent = ULyraManaComponent::FindManaComponent(LyraPC->GetPawn()))
					{
						ManaComponent->DamageSelfDestruct();
					}
				}
			}
		}
...
void ULyraCheatManager::UnlimitedMana(int32 Enabled)
{
	if (ULyraAbilitySystemComponent* LyraASC = GetPlayerAbilitySystemComponent())
	{
		const FGameplayTag Tag = LyraGameplayTags::Cheat_UnlimitedMana;
		const bool bHasTag = LyraASC->HasMatchingGameplayTag(Tag);
