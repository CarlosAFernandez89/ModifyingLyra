...
ULyraInventoryItemInstance* ULyraQuickBarComponent::AddItemFromInventory(ULyraInventoryItemInstance* ItemToAdd)
{
	int32 FreeItemSlotIndex = GetNextFreeItemSlot();
	if(FreeItemSlotIndex == INDEX_NONE) // If there is no slots empty
	{
		// Remove from current equipped slot
		ULyraInventoryItemInstance* RemovedItem = Slots[ActiveSlotIndex];
		UnequipItemInSlot();
		Slots[ActiveSlotIndex] = ItemToAdd;
		SetActiveSlotIndex(ActiveSlotIndex); 
		EquipItemInSlot();
		OnRep_Slots(); // Does calls to equip/un-equip
		return RemovedItem;
	}
	else // We put it in the free slot and equip it
	{
		Slots[FreeItemSlotIndex] = ItemToAdd;
		SetActiveSlotIndex(FreeItemSlotIndex);
		UnequipItemInSlot();
		EquipItemInSlot();
		OnRep_Slots();
	}
...
void ULyraQuickBarComponent::HolsterWeapon()
{
