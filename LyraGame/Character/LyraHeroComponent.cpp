...
void ULyraHeroComponent::HandleChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState)
{
	if (CurrentState == LyraGameplayTags::InitState_DataAvailable && DesiredState == LyraGameplayTags::InitState_DataInitialized)
	{
		...
		if (ULyraPawnExtensionComponent* PawnExtComp = ULyraPawnExtensionComponent::FindPawnExtensionComponent(Pawn))
		{
			...
			LyraAbilitySystemComponent = LyraPS->GetLyraAbilitySystemComponent();
			PawnExtComp->InitializeAbilitySystem(LyraAbilitySystemComponent, LyraPS);
		}
...
void ULyraHeroComponent::BeginPlay()
{
	...
	LockOnTarget = Cast<ULockOnTargetComponent>(GetOwner()->FindComponentByClass(ULockOnTargetComponent::StaticClass()));
		...
		if (const ULyraPawnData* PawnData = PawnExtComp->GetPawnData<ULyraPawnData>())
		{
			if (const ULyraInputConfig* InputConfig = PawnData->InputConfig)
			{
				...
				if (ensureMsgf(LyraIC, TEXT("Unexpected Input Component class! The Gameplay Abilities will not be bound to their inputs. Change the input component to ULyraInputComponent or a subclass of it.")))
				{
					...
					LyraIC->BindNativeAction(InputConfig, LyraGameplayTags::InputTag_Confirm, ETriggerEvent::Triggered, this, &ThisClass::Input_ConfirmTargeting,  /*bLogIfNotFound=*/false);
					LyraIC->BindNativeAction(InputConfig, LyraGameplayTags::InputTag_Cancel, ETriggerEvent::Triggered, this, &ThisClass::Input_CancelTargeting,  /*bLogIfNotFound=*/false);
...
void ULyraHeroComponent::Input_LookMouse(const FInputActionValue& InputActionValue)
{
	...
	if (Value.X != 0.0f)
	{
		...
		{
			LockOnTarget->SwitchTargetYaw(Value.X);
		}
		else
		{
	...
	if (Value.Y != 0.0f)
	{
		if(LockOnTarget->IsTargetLocked())
		{
			LockOnTarget->SwitchTargetPitch(Value.Y);
		}
		else
		{
...
void ULyraHeroComponent::Input_ConfirmTargeting(const FInputActionValue& InputActionValue)
{
	if(LyraAbilitySystemComponent)
	{
		LyraAbilitySystemComponent->InputConfirm();
	}
}

void ULyraHeroComponent::Input_CancelTargeting(const FInputActionValue& InputActionValue)
{
	if(LyraAbilitySystemComponent)
	{
		LyraAbilitySystemComponent->InputCancel();
	}
}
