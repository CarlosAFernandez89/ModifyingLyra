...
#include "LyraDamageManaExecution.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LyraDamageManaExecution)

struct FDamageManaStatics
{
	FGameplayEffectAttributeCaptureDefinition BaseDamageManaDef;

	FDamageManaStatics()
	{
		BaseDamageManaDef = FGameplayEffectAttributeCaptureDefinition(ULyraCombatSet::GetBaseDamageManaAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};

static FDamageManaStatics& DamageManaStatics()
{
	static FDamageManaStatics Statics;
	return Statics;
}


ULyraDamageManaExecution::ULyraDamageManaExecution()
{
	RelevantAttributesToCapture.Add(FDamageManaStatics().BaseDamageManaDef);
}

void ULyraDamageManaExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	...
	ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(FDamageManaStatics().BaseDamageManaDef, EvaluateParameters, BaseDamage);
	...
	const float ManaDamageDone = FMath::Max(BaseDamage * DistanceAttenuation * PhysicalMaterialAttenuation * DamageInteractionAllowedMultiplier, 0.0f);

	if (ManaDamageDone > 0.0f)
	{
		// Apply a damage modifier, this gets turned into - health on the target
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(ULyraManaSet::GetDamageAttribute(), EGameplayModOp::Additive, ManaDamageDone));
	}
