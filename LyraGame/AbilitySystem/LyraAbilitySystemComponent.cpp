...
TArray<UGameplayAbility*> ULyraAbilitySystemComponent::GetAbilitiesOfClass(UAbilitySystemComponent* AbilitySystem, TSubclassOf<UGameplayAbility> AbilityClass)
{
	if(!AbilitySystem)
	{
	...
	TArray<UGameplayAbility*> AbilitiesOfClass;
	
	const TArray<FGameplayAbilitySpec>& AbilitySpecArray = AbilitySystem->GetActivatableAbilities();

	for (const FGameplayAbilitySpec& Spec : AbilitySpecArray)
	{
		// Check if the ability is of the base class or a subclass
		if (Spec.Ability && Spec.Ability->IsA(AbilityClass))
		{
			AbilitiesOfClass.Add(Cast<UGameplayAbility>(Spec.Ability));
		}
	}

	return AbilitiesOfClass;
}
